# Used to validate and save rations array
class RationsUpdater
  def initialize(rations, user)
    ...
    @current_rations = DailyRation.select(:id, :quantity, :person_id,
                                          :daily_menu_id, :sprint_id, :dish_id
                                         )
  ...
  def insert!
    ...
    records = @rations.map do |item|
      DailyRation.new(price: item.price, quantity: item.quantity,
  ...
  def all_dishes_are_in_corresponding_menu?
    ...
    _by_days.each do |id, item|
      ...
      _avaliable = @daily_menus.find(id).dish_ids
  ...
  def all_dishes_have_correct_price?
    ...
    @rations.each do |_new|
      ...
      if _curr[1] != _new.price
        return "Dish with id #{_new.dish_id} has price #{_new.price} "\
               "but real price is #{_curr[1]}"
      end
    end
  ...
  def all_menus_prices_below_maximum?
    _new = @rations.group_by(&:daily_menu_id)

    _new.each do |id, rations|
      _sum = rations.reduce(0) { |a, e| a + e.price * e.quantity }
      if _sum > @daily_menus.find { |i| i.id == id }.max_total
